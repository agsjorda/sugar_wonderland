...
export class HelpScreen {
    ...
    private scrollInputZone?: Phaser.GameObjects.Zone;

    private scrollHandlers?: {
        update: () => void;
        wheel: (_pointer: any, _gameObjects: any, _deltaX: number, deltaY: number) => void;
    };
    
    // Game Rules section
    // RTP section
    // Payout section
    private readonly payoutSymbolCount: number = 11;
    ...
    public destroy(): void {
        const handlers = this.scrollHandlers;
        if (handlers) {
            try { this.scene.events.off('update', handlers.update); } catch {}
    ...
    private setupScrollableRulesContent(scene: GameScene): void {
        ...
        maskGraphics.fillRect(this.contentArea.x, this.contentArea.y, this.contentArea.width, this.contentArea.height);
        ...
        // Dedicated hit area for scrolling (drag / touch). This avoids depending on
        // container hit testing / depth ordering when other full-screen interactives exist.
        this.scrollInputZone = scene.add.zone(0, 0, this.contentArea.width, this.contentArea.height);
        this.scrollInputZone.setOrigin(0, 0);
        this.scrollInputZone.setScrollFactor(0);
        // Keep the zone non-interactive; scrolling is handled via global input events.
        try { this.scrollInputZone.disableInteractive(); } catch {}
        this.rulesContent.add(this.scrollInputZone);
    ...
    private createHelpScreenContent(scene: GameScene, contentArea: GameObjects.Container): void {
        ...
        const measuredHeight = this.measureContainerHeight(this.contentContainer);
        const finalHeight = Math.max(measuredHeight || 0, this.yPosition || 0);
        this.contentHeight = finalHeight + this.sectionSpacing / 2;
    }
    ...
    private loadHelpScreenImages(scene: GameScene, contentArea: GameObjects.Container): void {
        ...
        while (true) {
            ...
            // - "help_section{index}" (new help_screen folder assets)
    ...
    private enableScrolling(scene: Scene): void {
        // If this HelpScreen instance gets rebuilt for any reason, ensure we don't
        // stack multiple global input handlers.
        this.destroy();
        ...
        const debugScroll = ((): boolean => {
            try {
                return !!(globalThis as any).__DEBUG_MENU_SCROLL;
            } catch {
                return false;
            }
        })();
        ...
        // Ensure scroll view wins input hit-testing over the menu overlay.
        try { if ((this.scrollView as any).input) (this.scrollView as any).input.priorityID = 10; } catch {}
        ...
        const updateHandler = () => {
            if (!this.isMenuVisible() || !this.rulesContent.visible) {
                finishDrag();
                return;
            }

            const p = (scene as any).input?.activePointer as Phaser.Input.Pointer | undefined;
            if (!p) {
                finishDrag();
                return;
            }

            const top = this.menuTopPadding + this.tabHeight;
            const right = (scene as any).scale?.width ?? 0;

            if (!isDragging) {
                if (!p.isDown) return;
                // Start drag when pressing below the tab bar.
                // This avoids relying on container bounds which can be incorrect depending on transforms.
                if (p.y < top || (right > 0 && (p.x < 0 || p.x > right))) {
                    return;
                }
                ...
                if (debugScroll) {
                    try {
                        ...
                        console.log('[HelpScreen] drag start', {
                            x: p.x,
                            y: p.y,
                            viewHeight: this.contentArea.height,
            ...
            if (!hasExceededDragThreshold && Math.abs(p.y - startY) >= dragThreshold) {
            ...
            if (debugScroll) {
                try {
                    // eslint-disable-next-line no-console
                    console.log('[HelpScreen] drag move', {
                        deltaY,
                        currentY,
                        minY,
                        maxY,
        ...
        const wheelHandler = (pointer: any, _gameObjects: any, _deltaX: number, deltaY: number) => {
            if (!this.isMenuVisible() || !this.rulesContent.visible) return;
        ...
        if (debugScroll) {
            try {
                // eslint-disable-next-line no-console
                console.log('[HelpScreen] scrolling enabled', {
                    viewHeight: this.contentArea.height,
                    contentHeight: this.contentHeight,
                    yPosition: this.yPosition,
                });
    ...
    private ensureTabInteractionBlocker(scene: Scene): void {
        ...
        try { if ((blocker as any).input) (blocker as any).input.priorityID = 20; } catch {}
        if (blocker.input) {
