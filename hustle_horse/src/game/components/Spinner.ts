...
import { getSpinnerPosition, SPINNER_WHEEL_OFFSET_X, SPINNER_WHEEL_OFFSET_Y } from "../../config/UIPositionConfig";
...
export class Spinner {
  ...
  private createPortraitSpinner(scene: Scene, assetScale: number): void {
    ...
    const posA = getSpinnerPosition(scene);
    const scaleMultiplier = 0.378;
    this.container.setPosition(posA.x, posA.y);

    const wheelPosx = SPINNER_WHEEL_OFFSET_X;
    const wheelPosy = SPINNER_WHEEL_OFFSET_Y;
  ...
  private createLandscapeSpinner(scene: Scene, assetScale: number): void {
    ...
    const posB = getSpinnerPosition(scene);
    this.container.setPosition(posB.x, posB.y);
