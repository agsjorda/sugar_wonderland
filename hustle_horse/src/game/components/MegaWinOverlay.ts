...
/**
 * MegaWinOverlay - cloned from BigWinOverlay, but renders a "mega-win" title image.
 */
export class MegaWinOverlay {
    ...
    private titleKey: string = 'mega_win_title_png';
    ...
    public show(amount: number = 2500000.5, color: number = 0x0c2121, alpha: number = 0.92, durationMs: number = 700, onComplete?: () => void): void {
        ...
        // Use same music lock as BigWinOverlay, per "copy" requirement
        try {
            const audio = (window as any).audioManager;
            if (audio) {
        ...
        this.showBuildTimer = this.scene.time.delayedCall(120, async () => {
            ...
            if (titleReady) {
                try {
                    ...
                    console.warn('[MegaWinOverlay] Failed to create title image:', e);
                }
            }
    ...
    // Load Fire_Transition spine dynamically (same as BigWinOverlay)
    private loadFireTransitionIfNeeded(): Promise<boolean> {
        ...
        this.fireTransitionLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[MegaWinOverlay] fire transition dynamic load')) {
                ...
                console.warn('[MegaWinOverlay] Fire_Transition dynamic load failed:', e);
                this.fireTransitionLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    // Play Fire_Transition overlay covering the full viewport, then invoke callback (same as BigWinOverlay)
    private async playFireTransitionThenFinish(onFinished?: () => void): Promise<void> {
        if (!this.scene || !this.transitionContainer) return;
        if (!ensureSpineFactory(this.scene, '[MegaWinOverlay] fire transition factory')) {
            ...
            try {
                ...
                if (bw > 0 && bh > 0) {
                    ...
                    try {
                        ...
                        const sY = (this.transitionSpine.scaleY || (this.transitionSpine as any).scale || 1);
                        const topWorld = (this.transitionSpine.y || 0) + offY * sY;
                        const pad = Math.max(12, h * 0.02);
                        if (topWorld > -pad) {
                            const dy = topWorld + pad;
        ...
        return new Promise<boolean>((resolve) => {
            try {
                ...
                if (!(this.scene as any).textures.exists(this.titleKey)) {
                    ...
                    try { loader.image(this.titleKey, resolveAssetUrl(`${prefix}/dialogs/mega-win.png`)); } catch {}
                }
                const onComplete = () => {
                    try {
                ...
                console.warn('[MegaWinOverlay] Title dynamic load failed:', e);
    ...
    private loadFireSpineIfNeeded(): Promise<boolean> {
        ...
        this.fireLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[MegaWinOverlay] fire dynamic load')) {
                ...
                console.warn('[MegaWinOverlay] Fire spine dynamic load failed:', e);
                this.fireSpineLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    private loadMainFireIfNeeded(): Promise<boolean> {
        ...
        this.mainFireLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[MegaWinOverlay] main fire dynamic load')) {
                ...
                console.warn('[MegaWinOverlay] Main_Fire dynamic load failed:', e);
                this.mainFireLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    private async showMainFire(): Promise<void> {
        ...
        if (!ensureSpineFactory(this.scene, '[MegaWinOverlay] main fire factory')) return;
        ...
        try {
            ...
            console.warn('[MegaWinOverlay] Failed to show Main_Fire spine:', e);
        }
    }
    ...
    private tryCreateOverlayFireSpine(): boolean {
        if (!this.scene) return false;
        if (!ensureSpineFactory(this.scene, '[MegaWinOverlay] overlay fire factory')) return false;
        try {
            ...
            console.warn('[MegaWinOverlay] Failed creating overlay fire spine instance:', e);
            return false;
        }
    }
    ...
    public setFireScale(scale: number = 0.6): void {
        ...
        if (this.winFont) { try { (this.winFont as any).setScale?.(this.fireScale); } catch {} }
    }
    ...
    public destroy(): void {
        try {
            ...
            console.error('Error destroying MegaWinOverlay:', e);
        }
    }
}
