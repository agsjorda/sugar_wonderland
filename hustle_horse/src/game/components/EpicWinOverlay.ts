...
/**
 * EpicWinOverlay - cloned from BigWinOverlay, renders an "epic-win" title image.
 * This replaces the legacy SuperW_KA dialog.
 */
export class EpicWinOverlay {
    ...
    private titleKey: string = 'epic_win_title_png';
        ...
        this.showBuildTimer = this.scene.time.delayedCall(120, async () => {
            ...
            if (titleReady) {
                try {
                    ...
                    console.warn('[EpicWinOverlay] Failed to create title image:', e);
                }
            }
    ...
    private loadFireTransitionIfNeeded(): Promise<boolean> {
        ...
        this.fireTransitionLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[EpicWinOverlay] fire transition dynamic load')) {
                ...
                console.warn('[EpicWinOverlay] Fire_Transition dynamic load failed:', e);
                this.fireTransitionLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    private async playFireTransitionThenFinish(onFinished?: () => void): Promise<void> {
        ...
        if (!ensureSpineFactory(this.scene, '[EpicWinOverlay] fire transition factory')) {
        ...
        return new Promise<boolean>((resolve) => {
            try {
                ...
                if (!(this.scene as any).textures.exists(this.titleKey)) {
                    const prefix = this.getAssetPrefix();
                    try { loader.image(this.titleKey, resolveAssetUrl(`${prefix}/dialogs/epic-win.png`)); } catch {}
                }
                const onComplete = () => {
                    try {
                ...
                console.warn('[EpicWinOverlay] Title dynamic load failed:', e);
    ...
    private loadFireSpineIfNeeded(): Promise<boolean> {
        ...
        this.fireLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[EpicWinOverlay] fire dynamic load')) {
                ...
                console.warn('[EpicWinOverlay] Fire spine dynamic load failed:', e);
                this.fireSpineLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    private loadMainFireIfNeeded(): Promise<boolean> {
        ...
        this.mainFireLoadPromise = new Promise<boolean>((resolve) => {
            try {
                if (!ensureSpineLoader(this.scene!, '[EpicWinOverlay] main fire dynamic load')) {
                ...
                console.warn('[EpicWinOverlay] Main_Fire dynamic load failed:', e);
                this.mainFireLoadState = 'failed';
                resolve(false);
            }
        });
    ...
    private async showMainFire(): Promise<void> {
        ...
        if (!ensureSpineFactory(this.scene, '[EpicWinOverlay] main fire factory')) return;
        ...
        try {
            ...
            console.warn('[EpicWinOverlay] Failed to show Main_Fire spine:', e);
        }
    }
    ...
    private tryCreateOverlayFireSpine(): boolean {
        if (!this.scene) return false;
        if (!ensureSpineFactory(this.scene, '[EpicWinOverlay] overlay fire factory')) return false;
        try {
            ...
            console.warn('[EpicWinOverlay] Failed creating overlay fire spine instance:', e);
            return false;
        }
    }
    ...
    public destroy(): void {
        try {
            ...
            console.error('Error destroying EpicWinOverlay:', e);
        }
    }
}
