...
/**
 * Initialization Free Round Manager (Hustle Horse)
 *
 * Ported and adapted from Sugar Wonderland v2 so Hustle Horse can use
 * backend initialization data (free spin rounds) without changing the
 * existing scatter/bonus free‑spin flow.
 *
 * Responsibilities:
...
export class FreeRoundManager {
  ...
  public create(scene: Scene, gameAPI: GameAPI, slotController?: SlotController): void {
    ...
    // (match Sugar Wonderland v2 panel dimensions/positioning)
    // -------------------------------------------------------------------------
    const infoWidth = 125;        // Matches Sugar's balance-style panel
    const infoHeight = 55;
    const infoX = sceneWidth * 0.5;
    const controllerBandY = sceneHeight * 0.795;
    const infoY = controllerBandY;

    const infoBg = scene.add.graphics();
    const radius = 10;
    // Subtle gold frame with black interior, same visual style as Sugar
  ...
  private setupFreeRoundWinTracking(): void {
    gameEventManager.on(GameEventType.WIN_STOP, () => {
      ...
      try {
        ...
        // For hustle_horse, initialization free spins are base-game spins.
        // Prefer direct totalWin if available (matches Game scene logic).
        ...
        if (spinWin > 0) {
          ...
          console.log(
            `[FreeRoundManager] WIN_STOP (init freeround): added spinWin=$${spinWin}, accumulated=$${this.accumulatedFreeRoundWin}`
          );
        } else {
  ...
  private createRewardPanel(scene: Scene): void {
    ...
    // Match Sugar Wonderland v2 target size/position (~397w x 442h, centered)
    ...
    // "SPIN NOW" button using shared asset (same key/path as Sugar Wonderland)
