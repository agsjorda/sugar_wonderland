...
export class Buttons {
    ...
    public updateButtonStates(scene: GameScene): void {
		...
		let buyFeatureShouldDisable = shouldDisable || (this.autoplay?.isAutoPlaying ? true : false) || scene.gameData.isBonusRound;
    ...
    private createBet(scene: GameScene): void {
        ...
        try {
            ...
            const amplifyAnimIdle = scene.add.spine(width * 0.5, Buttons.PANEL_HEIGHT * 0.5, 'Amplify_Bet_Idle', 'Amplify_Bet_Idle') as SpineGameObject;
            amplifyAnimIdle.setScale(2);
            container.addAt(amplifyAnimIdle, 0);
            amplifyAnimIdle.setAlpha(0);

            this.amplifyBetIdle = amplifyAnimIdle;
            container.sendToBack(this.amplifyBetIdle);
        } catch (_e) {
        }
    ...
    private createDoubleFeature(scene: GameScene): void {
        ...
        const enhancedLabel = scene.add.text(width * 0.15, height * 0.15, 'ENHANCED BET', {
            fontSize: '12px',
            color: '#FFFFFF',
            fontFamily: 'Poppins',
            fontStyle: 'bold'
        }) as ButtonText;
        enhancedLabel.setFontSize('22px');
        enhancedLabel.setVisible(false);
        ...
        const toggleX = toggleWidth * 0.75;
    ...
    private createMarquee(scene: GameScene): void {
        ...
        youWonAmount.setOrigin(0.5, 0.8);
        const youWonText = scene.add.container(0, 0, [youWonLabel, youWonAmount]);
        container.add(youWonText);
