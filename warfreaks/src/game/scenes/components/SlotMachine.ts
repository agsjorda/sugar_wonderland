...
import { FreeSpinOverlayContainer } from './FreeSpinOverlayContainer';
import { CongratsOverlayContainer } from './CongratsOverlayContainer';
...
export class SlotMachine {
    ...
    private bombContainer: GameObjects.Container; // Separate container for bomb symbols (11-24)
    ...
    constructor() {
        this.symbolDisplayWidth = 160;
        this.symbolDisplayHeight = 160;
    ...
    preload(scene: Scene): void {
        ...
        if(this.isMobile){
            ...
            scene.load.image('bonusSlotBackground', 'assets/Reels/Mobile_Bonus_Grid.png');
        }
        else{
            scene.load.image('slotBackground', 'assets/Reels/Property 1=Default.png');
            scene.load.image('bonusSlotBackground', 'assets/Reels/Property 1=Variant2.png');
        }
    ...
    private initVariables(scene: GameScene): void {
        ...
        this.totalGridWidth = scene.scale.width;
    ...
    private createContainer(scene: GameScene): void {
        this.container = scene.add.container(
            this.isMobile ? scene.scale.width * 0.028 : this.slotX + scene.scale.width * 0.01,
            this.isMobile ? scene.scale.height * 0.25 : this.slotY + scene.scale.height * 0.01);
        this.container.setScale(this.isMobile ? 0.392 : 0.95);

        // Create separate container for bomb symbols (11-24) that bypasses the mask
        this.bombContainer = scene.add.container(
            this.isMobile ? scene.scale.width * 0.028 : this.slotX + scene.scale.width * 0.01,
            this.isMobile ? scene.scale.height * 0.25 : this.slotY + scene.scale.height * 0.01);
        this.bombContainer.setScale(this.isMobile ? 0.392 : 0.95);
        //this.bombContainer.setDepth(5); // Higher depth to appear above regular symbols

        this.totalGridWidth = this.isMobile ? this.totalGridWidth * 0.79 : this.totalGridWidth;
        this.totalGridHeight = this.isMobile ? this.totalGridHeight * 0.5 : this.totalGridHeight; 
    ...
    private createBackground(scene: GameScene): void {
        ...
        this.background = scene.add.image(0, 0, 'slotBackground').setOrigin(0, 0);
        this.bonusBackground = scene.add.image(0, 0, 'bonusSlotBackground').setOrigin(0, 0);
        this.background.setDepth(3);
        this.bonusBackground.setDepth(3);
        
        const paddingX = 90;

        this.background.x = -16;
        this.background.y = this.container.y / 1.5

        this.bonusBackground.x = this.background.x;
        this.bonusBackground.y = this.background.y;

        this.background.displayWidth = this.totalGridWidth + paddingX * 1.25 ;
        this.background.displayHeight = this.totalGridHeight * 1.01;

        this.bonusBackground.displayWidth = this.totalGridWidth + paddingX * 1.25 ;
        this.bonusBackground.displayHeight = this.totalGridHeight * 1.04;
    ...
    toggleBackground(scene: GameScene): void {
        if(scene.gameData.isBonusRound){
            this.background.setAlpha(0);
            this.bonusBackground.setAlpha(1);
        } else {
    ...
    private createSlot(scene: GameScene): void {
        ...
        const width = this.isMobile ? this.totalGridWidth + 90: this.totalGridWidth + 90;
        const height = this.isMobile ? this.totalGridHeight * 0.87 : this.totalGridHeight + 75*.20;
        
        const maskShape = scene.add.graphics();
        maskShape.fillRect(x, y, width, height);
        //maskShape.fillRoundedRect(x, y, width, height, 60);

        this.slotMask = maskShape.createGeometryMask();
        this.container.setMask(this.slotMask);
        this.toggleBombMask(false);
    ...
    toggleBombMask(isRemove: boolean){
        if(isRemove){
            this.bombContainer.clearMask();
        }
        else{
            this.bombContainer.setMask(this.slotMask);
        }
    }
    ...
    private async createReel(scene: GameScene, data: SpinData): Promise<void> {
        ...
        if (Array.isArray(apiFs) && apiFs.length > 0) {
            ...
            // After current spin concludes (entire tumble sequence done), animate scatters then show the free spins popup BEFORE toggling background
            Events.emitter.once(Events.TUMBLE_SEQUENCE_DONE, async (e: any) => {
                ...
                // Start spins after background toggle
                try {
                    if (scene.gameData.useApiFreeSpins && (this as any)?.startApiFreeSpins) {
                        (this as any).startApiFreeSpins(scene);
                    } else if (!scene.autoplay.isAutoPlaying) {
    ...
    private async playApiFreeSpin(scene: GameScene): Promise<void> {
        ...
        console.log("betForFs", betForFs, "thisFreeSpinTotal", thisFreeSpinTotal);
    ...
    private async playSpinAnimations(scene: GameScene, newValues: number[][], data: SpinData): Promise<void> {
        ...
        for (let col = 0; col < numCols; col++) {
            for (let row = numRows - 1; row >= 0; row--) {
                ...
                let symbolKey = 'Symbol0_WF'; // Default to scatter symbol
                ...
                if (symbolValue >= 0 && symbolValue <= 9) {
                    ...
                    if (symbolValue === 0) {
                        ...
                        if(symbolValue > 5){    
                            newSymbol.setSymbolDisplaySize(width * Slot.LOW_PAYING_SYMBOL_SIZE, height * Slot.LOW_PAYING_SYMBOL_SIZE);
                        }
                        else{
                            newSymbol.setSymbolDisplaySize(width * Slot.HIGH_PAYING_SYMBOL_SIZE, height * Slot.HIGH_PAYING_SYMBOL_SIZE);
                        }
                        scene.gameData.debugLog('Created SymbolContainer for symbol', { symbolValue, position: { x: symbolX, y: startY } });
                        
                    }
                } else if (symbolValue >= 10 && symbolValue <= 24) {
                ...
                if (symbolValue >= 10 && symbolValue <= 24) {
                ...
                if(newSymbol) {
                    ...
                    // Add bomb symbols to the bomb container (bypasses mask), regular symbols to main container
                    if (newSymbol instanceof BombContainer) {
                        this.bombContainer.add(newSymbol);
                    } else {
    ...
    private async handleScatterMatch(scene: GameScene, symbolGrid: number[][], scatterCount: number): Promise<string> {
        ...
        console.log("handleScatterMatch");
    ...
    private handleScatterRewards(scene: GameScene, scatterCount: number): void {
        ...
        if (scene.gameData.isBonusRound) {
            ...
            try { if (scene.autoplay.isAutoPlaying) { scene.autoplay.stop(); } } catch (_e) {}

            // Show the popup FIRST (before toggling background)
            this.showFreeSpinsPopup(scene, freeSpins);

            // After overlay is dismissed, toggle background/music and UI
            Events.emitter.once(Events.WIN_OVERLAY_HIDE, () => {
                try { scene.background.toggleBackground(scene); } catch (_e) {}
                try { scene.audioManager.changeBackgroundMusic(scene); } catch (_e) {}
                try {
                ...
                // Autoplay for local flow is started by the overlay on click if needed
            });
        }
    }
        ...
        matchedCells.forEach(({ row, col, symbol }) => {
            ...
            if (symbolSprite) {
                ...
                if (symbolSprite instanceof BombContainer) {
                    ...
                    // Use appropriate animation method based on symbol type
                    if (symbolSprite instanceof SymbolContainer) {
                        // Use the new playWinAnimation method for SymbolContainer
                        const animationPromise = symbolSprite.playWinAnimation()
                            .then(() => {
                                symbolSprite.destroy();
                            });
                        animationPromises.push(animationPromise);
                    } else if (symbolSprite instanceof GameObjects.Sprite) {
                        // Use playSymbolAnimation for regular sprites
    ...
    private async dropAndRefillAsync(symbolGrid: number[][], toRemove: boolean[][], scene: GameScene, SymbolsIn : Tumble): Promise<void> {
		...
		for (let col = 0; col < cols; col++) {
			...
			if (numIncoming > 0 && (landedCount === 100 || predictedTotal >= 100)) { // === 2 ; >= 3
            ...
            for (let i = 0; i < numNew; i++) {
                ...
                if (symbolValue >= 10 && symbolValue <= 22) {
                    ...
                    try { (sc.getSymbolSprite() as SpineGameObject).animationState.setAnimation(0, `animation`, false); } catch (_e) {}
                    if (symbolValue === 0) {
                        sc.setSymbolDisplaySize(width * Slot.SCATTER_SIZE, height * Slot.SCATTER_SIZE);
                    } else {   
                        if(symbolValue > 5){    
                            sc.setSymbolDisplaySize(width * Slot.LOW_PAYING_SYMBOL_SIZE, height * Slot.LOW_PAYING_SYMBOL_SIZE);
                        }
                        else{
                            // Regular symbols 1-9  
                            sc.setSymbolDisplaySize(width * Slot.HIGH_PAYING_SYMBOL_SIZE, height * Slot.HIGH_PAYING_SYMBOL_SIZE);
                        }
                    }
                    newSymbol = sc;
                }
    ...
    private createSampleSymbols(scene: GameScene, symbols: number[][]): void {
        ...
        for (let row = 0; row < numRows; row++) {
            ...
            for (let col = 0; col < numCols; col++) {
                ...
                const symbolX = centerX + col * (width + horizontalSpacing);
                const symbolY = centerY + row * (height + verticalSpacing);
                let symbol: GameObjects.Sprite | SymbolContainer | BombContainer | null = null;
                ...
                if (symbolValue >= 0 && symbolValue <= 9) {
                    ...
                    if (symbolValue === 0) {
                        ...
                        if(symbolValue > 5){    
                            symbol.setSymbolDisplaySize(width * Slot.LOW_PAYING_SYMBOL_SIZE, height * Slot.LOW_PAYING_SYMBOL_SIZE);
                        }
                        else{
                            // Regular symbols 1-9  
                            symbol.setSymbolDisplaySize(width * Slot.HIGH_PAYING_SYMBOL_SIZE, height * Slot.HIGH_PAYING_SYMBOL_SIZE);
                        }
                    ...
                    // Create BombContainer for bomb symbols (11-24)
                    symbol = new BombContainer(scene, symbolX, symbolY, symbolValue, scene.gameData);
                    if (symbol instanceof BombContainer) {
                        symbol.setBombDisplaySize(width * Slot.BOMB_SIZE_X, height * Slot.BOMB_SIZE_Y);
                    }
                    scene.gameData.debugLog('Created BombContainer for bomb symbol in createSampleSymbols', { symbolValue, position: { x: symbolX, y: symbolY } });
                }
                if (symbol) {
                    // Add bomb symbols to the bomb container (bypasses mask), regular symbols to main container
                    if (symbol instanceof BombContainer) {
                        this.bombContainer.add(symbol);
                    } else {
                        this.container.add(symbol);
                    }
                    this.symbolGrid[row][col] = symbol;
                // Animate the symbol using Animation.ts based on the symbolValue
                if (symbol instanceof SymbolContainer && this.animation && typeof this.animation.playSymbolAnimation === 'function') {
                    // Play the symbol animation (e.g., idle or intro) for the given symbolValue
                    this.animation.playSymbolAnimation(symbol, symbolValue);
                }
                }
            }
        }
                    ...
                    if (symbols[row][col] >= 1 && symbols[row][col] <= 9) {
                        ...
                        if (Math.random() < gameScene.gameData.bombChance / 100) { // Convert percentage to decimal
                            ...
                            if (rand < 0.6) { // Low (60%)
                                ...
                                bombType = Math.floor(Math.random() * 4) + 21; // 21-24
                            }
	...
	// Plays the Symbol0 intro animation at each scatter's position and resolves when all complete
	private async playSymbol0GridIntro(scene: GameScene): Promise<void> {
		return new Promise<void>((resolve) => {
			try {
				const animations: Promise<void>[] = [];
				for (let row = 0; row < this.symbolGrid.length; row++) {
					for (let col = 0; col < this.symbolGrid[row].length; col++) {
						const obj: any = this.symbolGrid[row][col];
						if (!obj) continue;
						let isScatter = false;
						let baseWidth = 0;
						let baseHeight = 0;
						let parent: any = obj.parentContainer || obj.parent || null;
						let localX = obj.x;
						let localY = obj.y;

						try {
							// Detect scatter based on type
							if (obj instanceof SymbolContainer) {
								isScatter = (obj as SymbolContainer).getSymbolValue?.() === 0;
								const inner: any = (obj as SymbolContainer).getSymbolSprite?.();
								if (inner && inner.displayWidth && inner.displayHeight) {
									baseWidth = inner.displayWidth;
									baseHeight = inner.displayHeight;
								}
							} else if ((obj as any).texture?.key === 'symbol0_WF') {
								isScatter = true;
								baseWidth = obj.displayWidth || 0;
								baseHeight = obj.displayHeight || 0;
							} else if ((obj as any).skeleton?.data?.name === 'Symbol0_WF') {
								isScatter = true;
								baseWidth = obj.displayWidth || 0;
								baseHeight = obj.displayHeight || 0;
							}
						...
						animations.push(new Promise<void>((done) => {
							try {
								let hostContainer: any = parent || scene.add.container(0, 0);
								if (!parent) {
									scene.add.existing(hostContainer);
								}
								const spine = scene.add.spine(localX, localY, 'Symbol0_WF', 'Symbol0_WF') as SpineGameObject;
								spine.setOrigin(0.5, 1);
								spine.setDepth(10001);
								try { (hostContainer as any).add?.(spine); } catch (_e) {}
								// Resize to match existing symbol footprint if available
								if (baseWidth > 0 && baseHeight > 0 && (spine as any).setDisplaySize) {
									(spine as any).setDisplaySize(baseWidth, baseHeight * 1.33) ;
								}
								// Hide original during animation to prevent overlap
								try { (obj as any).visible = false; } catch (_e) {}
								const track = spine.animationState?.setAnimation(0, 'symbol0_WF', false);
								let finished = false;
								if (track) {
									track.timeScale = 1;
									track.listener = {
										complete: () => {
											...
											try { (obj as any).visible = true; } catch (_e) {}
											done();
										}
									};
									// Failsafe timeout
									...
									// No track available; restore and finish quickly
								...
								// On error, continue
								scene.time.delayedCall(10, () => done());
							}
						}));
					}
				}
