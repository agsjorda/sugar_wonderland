...
interface GameScene extends Scene { 
    ...
    buyFeaturePopup: any; // Add this property to match Buttons.ts
}
    ...
    private spin(scene: GameScene, data?: SpinEventData): void {
        ...
        if (scene.gameData.freeSpins > 0) 
        {
            ...
            if (scene.gameData.freeSpins === 0) {
                ...
                // scene.buttons.toggleWinBar(scene);
                scene.slotMachine.toggleBackground(scene);
                // Change slot background back to main background
