...
- Returns the demo parameter value or `false` if not found
...
- **Implementation**: When currency symbols are removed, numeric values are centered within their containers instead of being offset to accommodate the symbol
- **Affected Elements**: Balance, bet amount, and feature button value texts are dynamically centered when in demo mode
...
- **Demo Check**: Checks for demo mode when creating tabs using `getDemoState()` only
...
##### `showHistoryContent()`
- **Demo Check**: Checks for demo mode before making API calls using `getDemoState()` only
...
##### `getHistory()`
- **Demo Check**: Checks for demo mode before making API call using `getDemoState()` only
