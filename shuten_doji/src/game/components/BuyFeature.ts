...
export class BuyFeature {
	...
	private createFeatureLogo(scene: Scene): void {
		...
		try 
		{
			...
			console.warn('[BuyFeature] Unable to create feature logo', error);

			// load a png instead
			this.featureLogo = scene.add.image(x, y, logoKey);
			this.featureLogo.setOrigin(0.5, 0.5);
			this.featureLogo.setScale(1);
		}

		const featureLogoBg = scene.add.image(x, y, 'buy_feature_logo_bg');
		featureLogoBg.setOrigin(0.5, 0.5);
		featureLogoBg.setScale(1);
