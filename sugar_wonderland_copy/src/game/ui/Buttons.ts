...
// remaining free spin label (mobile, desktop)
        ...
        Events.emitter.on(Events.HIDE_BOMB_WIN, () => {
            ...
            try{
                ...
                if (scene.gameData.useApiFreeSpins && fsLen > 0 && (scene.gameData.apiFreeSpinsIndex >= fsLen)) {
                    ...
                    text2.setText(`${scene.gameData.currency} ${finalTotal.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`);
                }
            } catch (_e) {}
        });
    }
    ...
    private createTotalWin(scene: GameScene): void {
        ...
        const text2 = scene.add.text(width * 0.5, Buttons.PANEL_HEIGHT * 0.65, `${scene.gameData.currency} ${0}`, {
            fontSize: '35px',
            color: '#FFFFFF',
            align: 'center',
            fontStyle: 'bold',
    ...
    private createBet(scene: GameScene): void {
        ...
        // Number of bets
        const selectSizeLabel = scene.add.text(padding, padding * 4, 'Select Bet Amount', {
            fontSize: '24px',
            color: '#FFFFFF',
            fontFamily: 'Poppins'
        });
        ...
        const buttons = betOptions.map((bet, index) => {
            ...
            const col = index % 5;
            const x = padding + col * (buttonWidth + spacing);
            const y = padding * 5.5 + row * (buttonHeight + spacing);
