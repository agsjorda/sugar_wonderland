...
export class LoadingPage extends Scene {
    ...
    preload(): void {
        ...
        // Create loading bars (global and center under spine)
        this.createLoadingBar();
        // this.createCenterLoadingBar();
    ...
    private createLoadingBar(): void {  
        ...
        this.load.on('progress', (value: number) => {
            ...
            // Update center bar
            if (this.centerLoadingBar && this.centerProgressText) {
                this.centerLoadingBar.clear();
                this.centerLoadingBar.fillStyle(0x222222, 0.8);
                this.centerLoadingBar.fillRoundedRect(this.centerBarX - 5, this.centerBarY - 5, barWidth + 10, height, borderRadius);
                this.centerLoadingBar.fillStyle(0x4CAF50, 1);
                this.centerLoadingBar.fillRoundedRect(this.centerBarX, this.centerBarY, barWidth * value, innerHeight, borderRadius);
                this.centerProgressText.setText(`${Math.floor(value * 100)}%`);
            }
        });         
        ...
        this.load.on('complete', () => {
            ...
            if (this.centerLoadingBar && this.centerProgressText) {
                ...
                this.centerLoadingBar.fillRoundedRect(this.centerBarX - 5, this.centerBarY - 5, barWidth + 10, height, borderRadius);
                this.centerLoadingBar.fillStyle(0x4CAF50, 1);
                this.centerLoadingBar.fillRoundedRect(this.centerBarX, this.centerBarY, barWidth, innerHeight, borderRadius);
                this.centerProgressText.setText('100%');
            }
        });
    }

    private createCenterLoadingBar(): void {
        const height = this.isMobile ? 26 : 30;
        const innerHeight = this.isMobile ? 18 : 22;
        const borderRadius = this.isMobile ? 10 : 12;
        const barWidth = this.isMobile ? 200 : 310;
        const fontSize = this.isMobile ? '16px' : '20px';

        // Position centered horizontally, a bit below screen center (beneath spine)
        this.centerBarX = this.cameras.main.centerX - barWidth / 2;
        this.centerBarY = this.cameras.main.centerY + (this.isMobile ? 110 : 140);

        this.centerLoadingBar = this.add.graphics();
        this.centerLoadingBar.fillStyle(0x222222, 0.8);
        this.centerLoadingBar.fillRoundedRect(this.centerBarX, this.centerBarY, barWidth, height, borderRadius);

        this.centerLoadingBar.fillStyle(0x4CAF50, 1);
        this.centerLoadingBar.fillRoundedRect(this.centerBarX, this.centerBarY, 0, innerHeight, borderRadius);

        this.centerProgressText = this.add.text(this.centerBarX + barWidth / 2, this.centerBarY + height / 2 - 2, '0%', {
            fontSize: fontSize,
            color: '#ffffff',
            fontStyle: 'bold',
            fontFamily: getFontFamily()
        ...
        // Ensure it sits above background but below overlayed UI
        try { this.centerLoadingBar.setDepth(101); (this.centerProgressText as any).setDepth?.(101); } catch {}
    }
