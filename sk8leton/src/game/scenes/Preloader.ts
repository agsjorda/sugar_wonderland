...
export class Preloader extends Scene
{
	...
	private createBaseLoadingView(screenConfig: any, assetScale: number): void
	{
		...
		// Show the intro Spine background scaled to cover
		try {
			if (ensureSpineFactory(this, '[Preloader] IntroAnimation_SK8')) {
				const introSpine: any = (this as any).add.spine(
					this.scale.width * 0.5,
					this.scale.height * 0.5,
					'IntroAnimation_SK8',
					'IntroAnimation_SK8-atlas'
				);
				try { introSpine.setOrigin?.(0.5, 0.5); } catch {}
				try { introSpine.setScrollFactor?.(0); } catch {}
				try { introSpine.setDepth?.(-2); } catch {}

				const applyCoverScale = () => {
					try {
						if (!introSpine || introSpine.destroyed) return;
						let b: any = null;
						try { b = introSpine.getBounds?.(); } catch { b = null; }
						const bw = Number(b?.width);
						const bh = Number(b?.height);
						if (!(isFinite(bw) && isFinite(bh) && bw > 1 && bh > 1)) return;
						const sx = this.scale.width / bw;
						const sy = this.scale.height / bh;
						const s = Math.max(sx, sy);
						try { introSpine.setScale?.(s); } catch { try { introSpine.scaleX = s; introSpine.scaleY = s; } catch {} }
						try { introSpine.x = this.scale.width * 0.5; introSpine.y = this.scale.height * 0.5; } catch {}
					} catch {}
				};

				try {
					const data: any = introSpine?.skeleton?.data;
					...
					if (animName) {
						try { introSpine.animationState?.setAnimation?.(0, animName, true); } catch {}
					}
				} catch {}

				try { this.time.delayedCall(0, applyCoverScale); } catch { try { applyCoverScale(); } catch {} }
				try {
		...
		// Add loading frame, text and website URL on top of the intro background (appears instantly)
	...
	preload () {
		try {
			this.load.on('loaderror', (file: any) => {
				try {
					const xhr = (file as any)?.xhrLoader?.xhr ?? (file as any)?.xhr ?? null;
					const status = xhr?.status ?? null;
					const url = (file as any)?.src ?? (file as any)?.url ?? null;
					console.error('[Preloader] Asset loaderror', {
						key: (file as any)?.key,
					...
					console.error('[Preloader] Asset loaderror (failed to inspect file)', e);
				}
			});
		} catch {}
		...
		try {
			...
			if (jsonCache?.has?.(spineKey)) {
				...
				if (defaultSkin && slots.length > 0) {
					...
					console.log('[Preloader] Patched Symbol1_Sk8 spine JSON slot keys (case normalization)');
				}
			}
		} catch (e) {
			console.warn('[Preloader] Failed to patch Symbol1_Sk8 spine JSON:', e);
		}
		...
		this.buttonSpin?.once('pointerup', () => {
			try {
				if (!audioStartedOnce) {
					...
					try { audio.playAmbience?.(500); } catch {}
				}
			} catch (e) {
				console.error('[Preloader] Error starting audio:', e);
			}
			try {
				this.buttonSpin?.disableInteractive();
				this.buttonSpin?.setTint(0x777777).setAlpha(0.9);
