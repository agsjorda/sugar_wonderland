...
export function flashRowSymbolsOverlay(host: SymbolFlashHost, rowIndex: number): void {
  try {
    ...
    for (let col = 0; col < host.symbols.length; col++) {
      ...
      try {
        const v = symbolData?.[col]?.[rowIndex];
        if (typeof v === 'number') {
          symbolValue = v;
        }
      } catch {}
      ...
      if (symbolValue === 5) {
        ...
        try {
          if (spinData) {
            variantKey = getSymbol5ImageKeyForCell(spinData, col, rowIndex);
          }
        } catch {}
      ...
      const w = Math.max(2, size?.w ?? host.displayWidth);
      const h = Math.max(2, size?.h ?? host.displayHeight);
      ...
      try {
        ...
        overlay.setDepth(Math.max(minDepth, 1005));
      } catch {}
      ...
      overlaysCreated++;
      flashedPositions.add(`${col}_${rowIndex}`);
    ...
    if (overlaysCreated > 0) {
      console.log(`[SymbolFlash] Row overlays created (row=${rowIndex}): ${overlaysCreated}`);
    }
  } catch {}
}
