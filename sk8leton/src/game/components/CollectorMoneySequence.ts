...
function waitSceneMs(scene: any, ms: number): Promise<void> {
	return new Promise<void>((resolve) => {
		try {
			const d = Math.max(0, Number(ms) || 0);
			if (scene?.time?.delayedCall) {
				scene.time.delayedCall(d, () => resolve());
				return;
			}
		} catch {}
		try {
...
async function incrementBonusGaugeProgress(scene: any): Promise<void> {
	try {
	...
	try {
		const gm: any = (scene as any)?.gaugeMeter;
		if (!gm) return;
		const p: any = (typeof gm.incrementActiveStageProgress === 'function')
			? gm.incrementActiveStageProgress()
			: (typeof gm.incrementStage1Progress === 'function' ? gm.incrementStage1Progress() : null);
		if (p && typeof p.then === 'function') {
			await Promise.race([
				Promise.resolve(p).catch(() => undefined),
				waitSceneMs(scene, 4000)
			]);
		}
	} catch {}
}

async function dissolveCollectorSymbol(scene: any, host: any, col: number, row: number, collectorObj: any): Promise<void> {
	try {
		if (!scene || !collectorObj) {
			try { host?.symbols?.[col] && (host.symbols[col][row] = null); } catch {}
			return;
		}

		try { clearNonWinningSymbolDim(scene, collectorObj); } catch {}
		try { clearImageSymbolWinRipple(scene, collectorObj); } catch {}
		try { scene?.tweens?.killTweensOf?.(collectorObj); } catch {}

		let speed = 1.0;
		try { speed = gameStateManager.isTurbo ? 0.65 : 1.0; } catch {}
		const duration = Math.max(90, Math.floor(180 * speed));

		const sx0 = (collectorObj.scaleX ?? 1) as number;
		const sy0 = (collectorObj.scaleY ?? 1) as number;

		try { (collectorObj as any).setVisible?.(true); } catch {}
		try { (collectorObj as any).visible = true; } catch {}

		await tweenTo(scene, collectorObj, {
			alpha: 0,
			scaleX: sx0 * 0.01,
			scaleY: sy0 * 0.01,
			duration,
		...
		try { (collectorObj as any).setVisible?.(false); } catch {}
		try { (collectorObj as any).visible = false; } catch {}
		try { (collectorObj as any).setActive?.(false); } catch {}
		try { (collectorObj as any).destroy?.(); } catch {}
		try {
			if (host?.symbols?.[col]?.[row] === collectorObj) {
				host.symbols[col][row] = null;
			}
		} catch {}
	} catch {}
}
	...
	try {
		...
		if (collectorOnly) {
			for (let collectorIndex = 0; collectorIndex < collectors.length; collectorIndex++) {
				...
				try { scene?.events?.emit?.('hook-collector-disintegrate-start'); } catch {}
				try {
					await dissolveCollectorSymbol(scene, host, collectorCell.col, collectorCell.row, collectorObj);
				} catch {}
				try {
					await incrementBonusGaugeProgress(scene);
				} catch {}
				try { scene?.events?.emit?.('hook-collector-complete'); } catch {}
				try {
					if (collectorObj) {
						try { clearImageSymbolWinRipple(scene, collectorObj); } catch {}
					}
