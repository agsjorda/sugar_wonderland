...
##### `getDemoState()`
- Returns `true` if `demo=true` in URL parameters, otherwise `false`
...
- **Demo Balance Update**: When demo mode is active and a win occurs in the **base game** (not scatter, not bonus), updates the demo balance:
  ...
  if (isDemo && !gameStateManager.isScatter && !gameStateManager.isBonus) {
      const totalWin = SpinDataUtils.getTotalWin(symbolsComponent.currentSpinData);
      if (totalWin > 0) {
...
#### UI Indicator
- **Demo Title Marker**: Appends `| DEMO` to the game title when demo mode is active (via `getDemoState()`), e.g. `That's Bait | DEMO`.
...
#### UI Indicator
- **Demo Title Marker**: Appends `| DEMO` to the game title when demo mode is active (via `getDemoState()`), e.g. `That's Bait | DEMO`.
  - Implemented when creating `ClockDisplay`

### 5. Menu.ts - History Tab
...
This checks URL parameters via `getDemoState()` which reads the `demo` parameter from the URL.
...
7. **History Tab Disabled**: History tab is visible but non-interactive in demo mode, with no history API calls made

## Constants Reference

| Constant | Type | Value | Usage |
|----------|------|-------|-------|
| `GAME_ID` | `string` | `'00020525'` | Game identifier in API requests |
...
### Currency Symbol Removal (Not Yet Implemented)
- In demo mode, all currency symbols (e.g., `$`, `£`) should be removed from the UI
- Value texts should be repositioned to account for the absence of currency symbols
- Affected components:
