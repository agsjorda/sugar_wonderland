...
export class ScatterAnimationManager {
  ...
  private showFreeSpinsDialog(data: Data): void {
    ...
    try {
      ...
      // Start looping the free spin dialog background sound after scatter animation completes
      try {
        const audioMgr = (window as any).audioManager;
        if (audioMgr && typeof audioMgr.startLoopingFreeSpinDialogSfx === 'function') {
          audioMgr.startLoopingFreeSpinDialogSfx();
          console.log('[ScatterAnimationManager] Started looping free spin dialog SFX after scatter animation');
        }
      } catch (e) {
        console.warn('[ScatterAnimationManager] Failed to start looping free spin dialog SFX:', e);
      }
