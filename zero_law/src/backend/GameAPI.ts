...
import { SLOT_COLUMNS, SLOT_ROWS, SLOT_COLUMN_HEIGHTS } from "../config/GameConfig";
...
export class GameAPI {  
    ...
    // Temporary development mode: no backend API available yet.
    // Always return the sample spin payload for every spin.
    // Disable via URL `?useApi=true` or localStorage `localStorage.setItem('useApi','true')`.
    private static readonly MOCK_SAMPLE_SPIN_EVERY_TIME: boolean =
        !(
            new URLSearchParams(window.location.search).get('useApi') === 'true' ||
            ...
            "tumbles": [
                {
                    "symbols": {
                        "in": [[6], [5], [1]],
                        "out": [
                            { "symbol": 2, "count": 4, "ways": 1, "win": 0.75 }
                        ]
                    },
                    "markers": [[0]],
                    "win": 0.75
                },
                {
                    "symbols": {
                        "in": [[4], [8], [8], [3, 7]],
                        "out": [
                            { "symbol": 5, "count": 4, "ways": 2, "win": 0.8 }
                        ]
                    },
                    "markers": [[1], [0, 0]],
                    "win": 0.8
    ...
    private createMockFirstManualScatterSpinData(bet: number): SpinData {
        ...
        const maxRows = Number(SLOT_COLUMNS || 5);

        // Variable-height columns (e.g. 3-4-5-5-4-3). Default to maxRows if missing.
        const heights: number[] =
            (Array.isArray(SLOT_COLUMN_HEIGHTS) && SLOT_COLUMN_HEIGHTS.length === cols)
                ? SLOT_COLUMN_HEIGHTS
                : Array.from({ length: cols }, () => maxRows);

        const area: number[][] = Array.from({ length: cols }, (_, col) => {
            const h = Math.max(0, Number(heights[col]) || maxRows);
            return Array.from({ length: h }, (_, row) => ((col * 3 + row) % 9) + 1);
        });

        // Place scatters (symbol id 0) on columns 0,1,2 at the middle visible position per-column
        for (const col of [0, 1, 2]) {
            if (col >= 0 && col < cols) {
                const h = area[col]?.length || 0;
                if (h > 0) {
                    const scatterRow = Math.max(0, Math.min(h - 1, Math.floor(h / 2)));
    ...
    public async doSpin(bet: number, isBuyFs: boolean, isEnhancedBet: boolean, isFs: boolean = false): Promise<SpinData> {
        // TEMP: Until a real backend API exists for this new game, always reuse the sample spin data.
        if (GameAPI.MOCK_SAMPLE_SPIN_EVERY_TIME) {
            const sample = JSON.parse(JSON.stringify(GameAPI.TEST_SPIN_DATA)); // Deep copy
            sample.bet = bet.toString();
            if (this.currentSpinData?.playerId) {
                sample.playerId = this.currentSpinData.playerId;
            }
            this.currentSpinData = sample as SpinData;
    ...
    public async simulateFreeSpin(): Promise<SpinData> {
        ...
        if (!freespinData || !Array.isArray((freespinData as any).items)) {
            throw new Error('No free spin data available. Please ensure SpinData contains freespin items.');
        }
        ...
        console.log('🎯 Remaining free spins:', (freeSpinData.slot as any)?.freespin?.count ?? (freeSpinData.slot as any)?.freeSpin?.count ?? 0);
