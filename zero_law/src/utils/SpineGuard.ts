...
export function ensureSpineFactory(scene: Scene, context: string): boolean {
	...
	const hasAttachedPluginInstance = () => {
		...
		// `isAtlasPremultiplied` is a stable public method on the SpinePlugin (see node_modules).
	...
	if (!hasFactory) {
		...
		try {
			...
			if (pluginMgr?.installScenePlugin) {
				// Use the same key as game config (`main.ts`) so Phaser de-dupes consistently.
