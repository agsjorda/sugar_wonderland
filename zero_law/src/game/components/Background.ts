...
export class Background {
	...
	private bgDefaultImage: Phaser.GameObjects.Image | null = null;
	...
	private createBackgroundLayers(scene: Scene, assetScale: number): void {
		// Add main background as a static image
		try {
			if (!scene.textures.exists('BG-Default')) {
				console.warn('[Background] BG-Default texture not loaded yet; background will be skipped');
			} else {
				this.bgDefaultImage = scene.add.image(
					scene.scale.width * 0.5,
					0,
					'BG-Default'
				).setOrigin(0.5, 0).setScale(assetScale);

				this.bgContainer.add(this.bgDefaultImage);
				console.log('[Background] Created BG-Default image background');
			}
		} catch (error) {
			console.error('[Background] Failed to create BG-Default image background:', error);
		}
    ...
    */
	...
	private createReelContainer(scene: Scene, assetScale: number): void {
		...
		const centerX = scene.scale.width * 0.46;
		...
		this.reelContainerImage = scene.add.image(
			...
			centerY,
			'reel-container'
		).setOrigin(0.5, 0.5).setScale(assetScale * 1).setDepth(1); // Behind symbols but above background spine
		...
		console.log('[Background] reel-container-frame is currently hidden');
	}
	...
	resize(scene: Scene): void {
		...
		// Keep background image pinned to the top of the screen
		if (this.bgDefaultImage) {
			this.bgDefaultImage.setPosition(scene.scale.width * 0.5, 0);
		}
	}
	...
	destroy(): void {
		...
		if (this.bgDefaultImage) {
			this.bgDefaultImage.destroy();
			this.bgDefaultImage = null;
		}
